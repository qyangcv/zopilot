...
| DOM-08 | Reader/Library pane、相邻区域与 splitter 宽度  | C    | 在不隐藏 Zotero 内容区的前提下，将 Zopilot 扩展到当前标准布局允许的最大宽度                                                            | probe 验证 pane、sibling、splitter 的直接关系、横向布局、连接状态与可解析 `min-width` | 隐藏最大化按钮；异常时仅恢复仍由 Zopilot 拥有的宽度          | 宽度计算、条件恢复、拖动释放、stacked 与双窗口 smoke | Zotero 提供公开 pane sizing API       |
...
- 最大化状态仅存在于当前主窗口内存；切换 surface、关闭、stacked、reload 与 shutdown
  均恢复宽度，原生 splitter 拖动则释放所有权并采用拖动结果。
...
- [ ] Reader/Library 最大化、窗口 resize、恢复及 splitter 拖动无宽度跳变；
