...
| API-06 | 插件自有 XUL chat window                       | B    | 提供不修改 Zotero pane/splitter 的宽屏聊天界面                                                                                         | `openDialog`、window document、插件自有 host 与 runtime factory 校验       | 保留原侧栏；记录 warning，不创建半挂载 React root            | 单例聚焦、关闭、主窗口卸载、reload/shutdown smoke    | Zotero 提供官方独立插件窗口 API       |
...
- 每个主窗口最多打开一个对应的 Zopilot 独立窗口；重复操作只聚焦已有窗口；
- 独立窗口只挂载插件自有 document，关闭、主窗口卸载、reload 与 shutdown 均释放 React
  root、portal、listener 和 window 引用。
...
- [ ] Reader/Library 打开独立窗口、重复聚焦、关闭及主窗口联动；
