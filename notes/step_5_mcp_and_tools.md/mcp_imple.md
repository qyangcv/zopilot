...
`paper_search` 不删除其能力，而是降级为 `ActivePaperRetrievalService` 内部的 retrieval 函数。`how_to_handle_pdf.md` 中关于 RAG、chunk、BM25、embedding、rerank、locator、warning/provenance 的经验仍然可用，但它们属于 `paper_read` 的内部实现，不再是模型可见的工具边界。
...
但此时 `paper_read` 可以先返回 metadata、abstract、fulltext status，不做 retrieval。

**验收标准**

- `tools/list` 返回且只返回 Step 5 公开工具。
- `tools/call get_active_paper` 在 active reader 中返回 scope。
...
## 3. 实现 get_active_paper

**目标**

提供一个独立的 scope 确认工具，让模型和开发者能确认“当前读的是哪篇论文”。

**在完整链路中的作用**

`get_active_paper` 不读取论文内容，因此不会和 `paper_read` 混淆。它主要用于：

- 模型在不确定 scope 时先确认当前 reader。
...
不接受 `itemId`、`path`、`libraryId` 作为输入，避免模型绕过当前 reader scope。

**输出结构**

```ts
type GetActivePaperOutput = {
  scope: {
    source: "reader";
    readerItemID: number;
...
3. 删除不应给模型的字段：
   ...
   - full text
   - hidden/debug object
4. 返回 compact text summary 和 structured output。
...
3. limits
   - 可保留全局安全上限和 warning，避免异常超长文本导致 app-server 或 UI 失控。
...
目标：实现 Zotero MCP server/tool registry 和 `get_active_paper`。

预期结果：Codex 能看到 `get_active_paper` 和占位 `paper_read`。

验收标准：无 active reader 和 active PDF reader 都稳定返回。
...
3. v1 不公开 `paper_search`，也不做 chunk/retrieve/rank；v2 的 retrieval/search 逻辑留在 `ActivePaperRetrievalService` 内部。
