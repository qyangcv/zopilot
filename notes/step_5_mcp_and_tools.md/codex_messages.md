...
- `item/agentMessage/delta` 的累积展示文本，并持续重渲染同一个 assistant bubble。
- MCP tool activity 后的下一段 assistant 文本；如果工具活动前已经有可见 assistant 文本，则在下一段 assistant 文本前插入一个 `---` 分隔符。连续多个工具事件只触发一个待插入边界。
...
`CodexBridge` 返回的 `result.text` 仍然是原始 `activeTurn.fullText.trim()`，不包含这些展示分隔符。
...
- `item/mcpToolCall/progress` 的具体内容；它只会作为工具活动信号影响后续 assistant 文本分隔。
- `item/started` / `item/completed` 中包含 `mcpToolCall` 的 tool lifecycle item 具体内容；它只会作为工具活动信号影响后续 assistant 文本分隔。
