...
- 你的当前 repo 还是 `zotero-plugin-template` 起步状态，`hooks.ts` 里基本都是 example factory：[hooks.ts](/Users/yang/code/zotero/zotero-copilot/src/hooks.ts:12)。可以直接从这里做一个轻量版，不需要继承 `llm-for-zotero` 那套庞大状态机。
