...
function retrieveContextCandidates(
  ...
  const resultLists = [
    searchField(index, plan.query, "text", "body search"),
    searchField(index, plan.query, "sectionPath", "section search"),
    searchField(index, plan.query, "caption", "caption search"),
    searchField(index, plan.query, "artifactLabel", "artifact search"),
  ];
  if (plan.locator) {
    resultLists.unshift(
      searchField(index, plan.locator.value, "artifactLabel", "locator exact"),
    );
    if (plan.locator.page) {
      resultLists.unshift(
        material.chunks
          .filter((chunk) => containsPage(chunk, plan.locator?.page))
          .map((chunk) => chunk.id),
      );
    }
  }
...
function structuralBoost(
  ...
  if (plan.locator?.page && containsPage(chunk, plan.locator.page)) {
    boost += 0.2;
  }
  if (plan.locator) {
    const locatorLabel =
...
function containsPage(chunk: MaterialChunk, page: number | undefined): boolean {
  return (
    page !== undefined &&
